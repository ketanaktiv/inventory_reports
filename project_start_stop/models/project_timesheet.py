...
class AccountAnalyticLine(models.Model):
    ...
    last_update = fields.Date(
        compute="_last_update_date", string="Last Update Date", store=True)
    ...
    @api.multi
    def _get_hours(self):
        """Function for Calculate hours two time duration."""
        for obj in self:
            if obj.stop_time and obj.start_time:
                worked_time = datetime.strptime(
                    obj.stop_time, '%Y-%m-%d %H:%M:%S') - datetime.strptime(
    ...
    def get_hours(self, start_time, stop_time):
        """Function use for calculate hours."""
        if not start_time or not stop_time:
            return 00.00
        worked_time = datetime.strptime(
            stop_time, '%Y-%m-%d %H:%M:%S') - datetime.strptime(
            start_time, '%Y-%m-%d %H:%M:%S')
        worked_time_seconds = worked_time.total_seconds() / 3600.0
        return float(worked_time_seconds)

    def get_allowed_user_list(self, obj):
        """Here obj must be single object."""
        user_list = []
        try:
            for users in obj.user_ids:
                if obj and users:
    ...
    def check_allowed_user(self, obj):
        """Function is design to check employee
        working on one task at a time."""
        model_name = obj._name
        for users in obj.user_ids:
            if obj and (users.id not in self.get_allowed_user_list(obj)):
                vals = {}
                ...
                return {
                    'name': _("Warning"),
                    'view_mode': 'form',
                    'view_id': self.env['ir.model.data'].xmlid_to_object(
                        'project_start_stop.active_work_log_message_wizard').id,
                    'view_type': 'form',
    ...
    def check_current_work(self):
        """Function is design to check whether the employee working
         on other task."""
        active_work_log_obj = self.env["account.analytic.line"].search(
            [('user_id', '=', self._uid), ('is_log_active', '=', True)])
    ...
    def check_all_info_for_log(self, obj):
        """Function Use for check log info."""
        res = {}
        if obj:
            for o in obj:
                ...
                if o.log_action != "not_working":
                    raise ValidationError(
                        _('''You can not start "%s" because this task has
                         been already closed. You need to reactivate it
                          first.''') % o.name)
    ...
    @api.multi
    def work_log_start(self, obj, group_entry=True):
        """Function use for start work."""
        if obj:
            if group_entry:
            ...
            for users in user_ids:
                try:
                    ...
                    vals = {
                        'user_id': users.id,
                        'start_time': datetime.today().strftime(
                            '%Y-%m-%d %H:%M:%S'),
                        'is_log_active': True,
                    ...
                    if model_name == "project.task" and not obj.project_id:
                        raise UserError(
                            _("""Task which you are going to start has no
                                project.So you can't start working on task now.
                                Assign project first then try again. """))
                    else:
                        raise UserError(_("""You are not authorized to read associated project.
                            So contact your team leader to add you as
                            team memebr to associated project then try again.
                            """))
                    if model_name == "project.issue" and not obj.project_id:
                        raise UserError(_("""Issue which you are going to start has no
                            project. So you can't start working on issue now.
                            Assign project first then try again. """))
                    else:
                        raise UserError(_("""You are not authorized to read associated
                            project.So contact your team leader to add you as
                            team memebr to associated project then try again.
                            """))
        return work_log_obj
        ...
        for obj in objs:
            ...
            if model_name == "project.task":
                if not self.check_allowed_user(obj=obj):
                    task_work_log_obj = self.search(
                        [("task_id", "=", obj.id), (
                            "is_log_active", "=", True)])
                    for task in task_work_log_obj:
                        if task:
                            current_time = str(
                            ...
                            start_time = datetime.strptime(
                                ...
                                '%Y-%m-%d %H:%M:%S')
                            total_spent_hours = (
                                stop_time - start_time).total_seconds() / 60.0 / 60
                            task.write({
                                'stop_time': datetime.today().strftime(
                                    '%Y-%m-%d %H:%M:%S'),
                                'is_log_active': False,
                                "name": task.wk_name_get(),
            ...
            if model_name == "project.issue":
                if not self.check_allowed_user(obj=obj):
                    issue_work_log_obj = self.search(
                        [("issue_id", "=", obj.id), (
                            "is_log_active", "=", True)])
                    if issue_work_log_obj:
                        ...
                        stop_time = datetime.strptime(
                            current_time, '%Y-%m-%d %H:%M:%S')
                        start_time = datetime.strptime(
                            issue_work_log_obj.start_time,
                            '%Y-%m-%d %H:%M:%S')
                        total_spent_hours = math.ceil(
                            float((
                                stop_time - start_time).seconds / 60)) / 60
    ...
    def wk_name_get(self):
        """Function use for get description for task."""
        for log in self:
            if not log.stop_time:
                user_tz = self._context.get(
                    "tz", 'Asia/Kolkata') or 'Asia/Kolkata'
                ...
                if date_start == date_stop:
                    ...
                    name = "Worked on (%s To %s)" % (
                        ...
                        datetime.strftime(
                            pytz.utc.localize(
                                datetime.strptime(datetime.today().strftime(
                                    '%Y-%m-%d %H:%M:%S'), DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local),
                            "%d-%B-%Y %H:%M:%S")
                    )
                    return name
            else:
    ...
    def get_task_time_duration(self, task_ids):
        """Function calculates the total time spent by employee(s)."""
        ...
        for task_obj in self.env['project.task'].browse(task_ids):
            ...
            wizard_id = self.with_context(context).env['work.log.time.wizard'].create(
                {'task_id': task_ids[0], 'total_time': message})
            return {
                'name': _("Time Log Report"),
                'view_mode': 'form',
                'view_id': self.env['ir.model.data'].xmlid_to_object(
                    'project_start_stop.work_total_hours_message_wizard').id,
                'view_type': 'form',
